...
export async function GET(
  ...
  try {
    ...
    const isMember = groupTask.group.users.some(userGroup => userGroup.userId === decoded.id)
    if (!isMember) {
      return NextResponse.json({ error: 'Forbidden' }, { status: 403 })
    }

    // Fetch all task logs for this group task, ordered by creation time
    ...
    console.log(`[DEBUG] Calculated time for group task ${groupTaskId}: ${totalTimeSeconds}s from logs, stored timeSum: ${groupTask.timeSum}s`)

    return NextResponse.json({
      calculatedTimeSeconds: totalTimeSeconds,
      storedTimeSum: groupTask.timeSum,
      logs: logs.map(log => ({
        id: log.id,
        logType: log.logType,
    ...
    console.error('Error calculating group task time from logs:', error)
    return NextResponse.json(
